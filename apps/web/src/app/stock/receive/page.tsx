...
export default function ReceiveStockPage() {
  ...
  async function loadData() {
    try {
      const [e, s, v] = await Promise.all([
        api.get<StockEntry[]>('/stock/entries?type=RECEIVED').catch(() => [] as StockEntry[]),
  ...
  return (
    <div>
      <Topbar title="Receive Stock" subtitle="Log incoming stock deliveries" />
      <div className="p-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {entries.length === 0 ? (
              <p className="text-slate-500 text-sm text-center py-8">No received stock entries</p>
            ) : (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <PackagePlus size={14} className="text-green-400" />
                      ...
                      <p className="text-slate-400 text-sm">
                        {entry.location || 'No location specified'}
                      </p>
                      {entry.notes && (
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Receive Stock">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input
            ...
            placeholder="Warehouse A, Landing site..."
          />
          <Input
            label="Notes"
            value={form.notes}
